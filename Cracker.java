...
public class Cracker {
	...
	public static void main(String[] Args) throws IOException {
		...
		try{
			Scanner shadowFile = new Scanner(new File("shadow.txt"));
			while(shadowFile.hasNext()){
				String shadowFileLine = shadowFile.next();
				
				String[] lineSplittedVals = shadowFileLine.split(":");
				String userName = lineSplittedVals[0];
				String passwordDetails = lineSplittedVals[1];
				String[] passwordDetailsList = passwordDetails.split("\\$");
				String userSalt = passwordDetailsList[2];
				String originalPasswordHash = passwordDetailsList[3];
				
				for (String commonPassword: passWordsList) {
					String commonPasswordHash = null;
					if (commonPassword.length()>16) {
						continue;
					}
					else {
						commonPasswordHash = MD5Shadow.crypt(commonPassword, userSalt);
					}
